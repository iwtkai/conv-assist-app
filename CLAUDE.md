...
**プロキシ経由の API 呼び出し**：`WORKER_URL` 定数（App.jsx:22）が Cloudflare Worker のエンドポイント。フロントから直接 Anthropic API を叩かないのは API キー保護のため。Worker URL を変更する場合はここだけ修正すればよい。

**音声認識の再初期化**：`SpeechRecognition` インスタンスは `lang` と `autoAnalyze` の変更で再生成される（useEffect の依存配列）。音声認識まわりを変更する場合は、`recognitionRef`・`silenceTimer`・`langRef`・`stopRequestedAtRef`・`stopTimeoutRef` の連携に注意。録音中に `lang`/`autoAnalyze` を変更する操作（言語タブ切替・自動解析トグル）は、`stopForReconfigure()` を経由して停止すること（直接 `recognitionRef.current.stop()` を呼ぶと「停止処理中」の最低表示時間が保証されない）。
